...
let
  ...
  droidSrc = fetchurl {
    ...
    sha256 = hashes.droid.${stdenv.hostPlatform.system};
  };

  rgSrc = fetchurl {
    url = "${baseUrl}/ripgrep/${platform}/${arch}/rg";
    sha256 = hashes.rg.${stdenv.hostPlatform.system};
  };
in
stdenv.mkDerivation {
  pname = "factory-cli";
  ...
  installPhase = ''
    ...
    local rg_dir="$out/libexec/factory-cli"
    install -Dm755 ${rgSrc} "$rg_dir/rg"
    makeWrapper $out/bin/droid $out/bin/droid \
      --prefix PATH : "$rg_dir"
    runHook postInstall
  '';
